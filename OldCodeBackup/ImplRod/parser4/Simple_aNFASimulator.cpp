...
static std::string createString(aNFAnode* E, int target, char c, int from) {

  if(E->nr == 3 && from == 3) {
    std::cout << "Doing stuff " << target << std::endl;
  }
  std::cout << "Doing stuff " << target << " from " << from << " at " << E->nr << std::endl;
  // Is this the node we are looking for?
  if(!c)
    std::cout << "Looking for epsilon\n";
  if(!c && E->nr == target) {
    if(E->i2 != NULL) {
      if(from == E->i1->nr) {
        return "0";
  ...
  if(c && E->right == NULL) {
    if(E->input == c || inCharClass(c, E->charClass)) {
        return createString(E->left, target, '\0', E->nr);
    }
      if(E->i2 != NULL) {
        if(E->i1->nr == from)
          return "0" + createString(E->left, target, c, E->nr);
        if(E->i2->nr == from)
          return "1" + createString(E->left, target, c, E->nr);
      }
      return createString(E->left, target, c, E->nr);
  }

  // A split path, return the best string
  if(E->right != NULL) {
    std::string tmp =  createString(E->left, target, c, E->nr);
    std::string tmp2 = createString(E->right, target, c, E->nr);
    if(tmp == "na") {
      if(tmp2 == "na") {
        return tmp2;
      }
 ...
 // // Do we read our character when passing through this node?
 // if(c && E->i2 == NULL) {
 //   if(E->left->input == c || inCharClass(c, E->left->charClass)) {
 //       return createString(E->i1, target, '\0');
 //   }
 //     return createString(E->i1, target, c);
 // }
 //
 // // A split path, return the best string
 // if(E->i2 != NULL) {
 //   std::string tmp =  createString(E->i1, target, c);
 //   std::string tmp2 = createString(E->i2, target, c);
 //   if(tmp == "na") {
 //     if(tmp2 == "na") {
 //       return tmp2;
 //     }
...
static void buildMatrix(aNFAnode* E, int numStates, char c,  std::string** matrix) {
  ...
  for(int i = 0; i < numStates; i++) {
    ...
    for(int j = 0; j < numStates; j++) {
      //tmp = findN(E, j, numStates + 1);
      *retMat[i*numStates + j] = createString(tmp, j, c , i);
    }
  }
}
...
std::string* s_simulate(std::string regEx, std::istream* input) {
  ...
  for(int i = 0; i < numStates; i++) {
    S[i] = new std::string();
    *S[i] = createString(initialState, i, '\0', 0);
  }
  
  for(int i = 0; i < language.size(); i++) {
    printMatrix(allM + i*sizeof(std::string*)*sizeof(std::string*), numStates);
  }
