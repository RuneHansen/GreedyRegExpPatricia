...
*/
  ...
  int length = s.length < next.bitstring.length ? next.bitstring.length : s.length;

  int i = 0; //length of langest common prefix
  for (i; i < length; i++) {
    if (s[i] != next.bitstring[i]) {
      break;
    }
  }
  ...
  if (i == child.bitstring.length && child.bitstring.length < s.length) {
    return insertPat(next, s->substr(i));
  }

    next2 = calloc(1, sizeof(patNode));
    next2->bitstring = s->substr(0,i);
    next2->left = NULL;
    next2->right = NULL;
    next2->parent = node;
  ...
  //new node with common prefix as string and remainder of each as children
    next3 = calloc(1, sizeof(patNode));
    next3->bitstring = s->substr(i);
    next3->left = NULL;
    next3->right = NULL;
    next3->parent = next2;
...
void update(patNode** S, const int numStates, std::string** m) {
  ...
  patNode** newS = (patNode**) calloc(numStates, sizeof(patNode*) * numStates);
...
std::string* simulate(std::string regEx, std::string test_input) {
  ...
  patNode* root = calloc(1, sizeof(patNode));

  patNode** S = (patNode**) malloc(sizeof(patNode*) * numStates);
  for(int i = 0; i < numStates; i++) {
    S[i] = insertPat(root, &createString(initialState, i, '\0'));
  }
